    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.games.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('team_1') ? 'is-invalid' : '' }}" name="team_1_id" id="team_1_id">
                    @foreach($team_1s as $id => $team_1)
                        <option value="{{ $id }}" {{ old('team_1_id') == $id ? 'selected' : '' }}>{{ $team_1 }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('team_2') ? 'is-invalid' : '' }}" name="team_2_id" id="team_2_id">
                    @foreach($team_2s as $id => $team_2)
                        <option value="{{ $id }}" {{ old('team_2_id') == $id ? 'selected' : '' }}>{{ $team_2 }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('result_1') ? 'is-invalid' : '' }}" type="number" autocomplete="off"  name="result_1" id="result_1" value="{{ old('result_1', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('result_2') ? 'is-invalid' : '' }}" type="number" autocomplete="off" name="result_2" id="result_2" value="{{ old('result_2', '') }}" step="1">
